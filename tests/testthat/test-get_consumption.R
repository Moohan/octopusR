...
test_that("Correctly handles multi-page parallel requests", {
  ...
  mock_api_multi_page <- function(path, query, ..., perform = TRUE) {
    if (perform) {
      ...
      # Return a real request object as expected by refactored get_consumption
  ...
  mock_req_perform_parallel <- function(reqs, ...) {
    lapply(reqs, function(req) {
      # req is a real httr2_request object
      # Extract page from the request url
      url_parts <- httr2::url_parse(req$url)
      page_num <- as.integer(url_parts$query$page)
