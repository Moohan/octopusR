...
#' `x[is.na(x)] <- 0`) provides a significant performance boost (~3-4x speedup
#' and ~70% memory reduction in this package) compared to
#' `ifelse(is.na(x), 0, x)` in `combine_consumption`, as verified by
#' `bench::mark()`.
#' @export
