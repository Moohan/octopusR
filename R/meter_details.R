...
get_meter_details <-
  function(meter_type = c("electricity", "gas"),
    ...
    if (is_testing()) {
      meter <- testing_meter(meter_type)
      if (isFALSE(include_gsp)) {
        meter[["gsp"]] <- NULL
      }
      return(meter)
    }
    ...
    if (!identical(mpan_mprn, "") && !identical(serial_number, "")) {
      ...
      if (meter_type == "electricity") {
        if (isTRUE(include_gsp)) {
          meter[["gsp"]] <- get_meter_gsp(mpan = mpan_mprn)
        } else {
          meter[["gsp"]] <- NA
        }
      }
...
testing_meter <- function(meter_type = c("electricity", "gas")) {
  ...
  if (meter_type == "electricity") {
    ...
    mpan <- sanitize_derived_string(mpan, "sk_test_mpan")
    ...
    serial_number <- sanitize_derived_string(serial_number, "sk_test_serial")
    ...
    mprn <- sanitize_derived_string(mprn, "sk_test_mprn")
